...
import com.jzh.car.service.RedisService;
import com.jzh.car.component.CancelOrderSender;
import com.jzh.car.mapper.PortalOrderMapper;
import com.jzh.car.mapper.PortalOrderItemMapper;
import com.jzh.car.domain.CartPromotionItem;
import com.jzh.car.domain.ConfirmOrderResult;
import com.jzh.car.domain.OrderParam;
...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    @Resource
    private SmsCouponHistoryMapper couponHistoryDao;
    @Resource
    private OmsOrderMapper orderMapper;
    @Resource
    private PortalOrderItemMapper orderItemDao;
    @Resource
    private com.jzh.car.mapper.SmsCouponHistoryMapper couponHistoryMapper;
    @Resource
    ...
    public Integer paySuccess(Long orderId, Integer payType) {
        ...
        for (OmsOrderItem orderItem : orderDetail.getOrderItemList()) {
            int count = portalOrderMapper.reduceSkuStock(orderItem.getProductSkuId(), orderItem.getProductQuantity());
            if (count == 0) {
    ...
    public void cancelOrder(Long orderId) {
        ...
        if (cancelOrder != null) {
            ...
            if (!CollectionUtils.isEmpty(orderItemList)) {
                for (OmsOrderItem orderItem : orderItemList) {
                    int count = portalOrderMapper.releaseStockBySkuId(orderItem.getProductSkuId(), orderItem.getProductQuantity());
                    if (count == 0) {
    ...
    private void lockStock(List<CartPromotionItem> cartPromotionItemList) {
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList) {
            ...
            int count = portalOrderMapper.lockStockBySkuId(cartPromotionItem.getProductSkuId(), cartPromotionItem.getQuantity());
            if (count == 0) {
                Asserts.fail("库存不足，无法下单");
            }
        }
